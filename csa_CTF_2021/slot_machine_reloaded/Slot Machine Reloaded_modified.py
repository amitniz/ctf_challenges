...
class Slotmachine(object):
    ...
    def choice(self):
        rand_num = format(self.random._randbelow(6277101735386680763835789423207666416102355444464034512895),'#0%db' % 194)[2:]
        result = ""
        j = 0
        for i in range(0,192,6):
            result += self.slots[j][int(rand_num[i:i+6],2)] #64 possibility (choose letter)
            j += 1
        return result
