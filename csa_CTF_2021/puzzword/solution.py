...
def solve(steps,board,solution,init_time,x_order=1):
    if board == solution:
        return 1,steps
        ...
        for x in range(N_X):
            for y in range(N_Y):
                for direction in directions: 
                    if can_move(x,y,direction,board):
                        step,new_board = move(x,y,direction,board)
                        steps.append(step)
                        #print_board(new_board)
                        res,steps = solve(steps,new_board,solution,init_time)
                        if res ==1:
                            return 1,steps
                        elif res ==-1:
                            return -1,steps
...
def random_solve(steps,board,init_time,order):
    if board == solution:
        return 1,steps
    ...
    for pos in order:
        for direction in directions: 
            if can_move(pos[0],pos[1],direction,board):
                ...
                steps.append(step)
                #print_board(new_board)
                res,steps = random_solve(steps,new_board,solution,init_time,order)
                if res ==1:
                    return 1,steps
                elif res ==-1:
                    return -1,steps
...
try:
    while 1:
        ...
        if steps is None:
            ...
            s,steps =solve([],src_board,dst_board,time.time())
            if s==1:
